...
func main() {
	...
	// Create database tables with proper dependency order
	...
	// Public problem viewing endpoints (no auth required)
	r.Route("/api/problems", func(problemsRouter chi.Router) {
		problemsRouter.Get("/", problemsHandler.GetProblems)
		problemsRouter.Get("/{id}", problemsHandler.GetProblem)

		// Protected problem creation (requires auth)
		problemsRouter.Group(func(protected chi.Router) {
			protected.Use(commonauth.RequireAuth(jwtSecret, logger))
			protected.Post("/", problemsHandler.CreateProblem)
			protected.Post("/{id}/testcases", problemsHandler.CreateTestCase)
		})
	})
	...
	r.Route("/api", func(apiRouter chi.Router) {
		...
		apiRouter.Get("/submissions/{id}", submissionsHandler.GetSubmission)
