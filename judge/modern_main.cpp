...
public:
    ...
    bool compile_submission(const Submission &submission, const std::string &executable_path)
    {
        ...
        // Compile using system call (not sandboxed - compilation is trusted)
        std::string compile_command = "/usr/bin/g++ " + source_path + " -o " + executable_path +
                                      " -std=c++17 -O2 2>&1";
    ...
    {
        const char *query = "UPDATE submissions SET verdict = $1 WHERE id = $2";
